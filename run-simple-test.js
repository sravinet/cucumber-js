...
// Run vitest with the simple configuration, targeting only the simple.test.ts and cucumber-test.ts files
console.log(`Running vitest with: ${vitestPath} run test/simple.test.ts test/cucumber-test.ts`);
try {
  if (vitestPath.endsWith('.js') || vitestPath.endsWith('.mjs')) {
    execSync(`node ${vitestPath} run test/simple.test.ts test/cucumber-test.ts`, { stdio: 'inherit' });
  } else {
    execSync(`${vitestPath} run test/simple.test.ts test/cucumber-test.ts`, { stdio: 'inherit' });
  }
} catch (error) {
